...
class AppearanceOptModule(torch.nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            embed_ids: (C,) embedding indices. During training, this is the
                training image index. During evaluation, this should be the
                index of the nearest training camera (by pose distance).
            dirs: (C, N, 3)
...
def normal_rgb2sn(normals, output_file):
    # Swap the 1st and 3rd components
    a = np.copy(normals[:,:,0])
    b = np.copy(normals[:,:,2])
    normals[:,:,0] = b
    normals[:,:,2] = a
...
# Experimental
@torch.no_grad()
def save_ply(path, splats, is_2dgs=True):
    ...
    for i in tqdm(range(2 * num_blocks - 1)):
        for j in range(2 * num_blocks - 1):
            for k in range(2 * num_blocks - 1):
                # Define the range of each block
                block_min = mins + np.array([i, j, k]) / 2 * block_size
                block_max = block_min + block_size
                # Find the points that fall within this block
                block_points = points[
                ...
                if block_points.shape[0] > 10:  # If this block contains points
